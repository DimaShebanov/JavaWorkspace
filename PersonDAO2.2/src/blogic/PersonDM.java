...
import dal.PersonDAO_JSonManual;
import dal.PersonDAO_Mock;
import dal.PersonDAO_XMLManual;
import dal.PersonDAO_YAMLManual;
...
public class PersonDM extends AbstractTableModel
{
	...
	public int getColumnCount() 
	{
		Person p = new Person();
		Class cp = p.getClass();
		Field[] fld = cp.getDeclaredFields();
	...
	public Object getValueAt(int row, int column) 
	{
		...
		Class cp = p.getClass();
		Field[] fld = cp.getDeclaredFields();
		String str = getColumnName(column).toLowerCase();
		for (Field field : fld)
		{
	...
	class ActionComboBoxListener implements ActionListener
	{
		...
		public void actionPerformed(ActionEvent e) 
		{
			...
			switch ((String)box.getSelectedItem()) 
			{
				...
				case "XML":   pd = new PersonDAO_XMLManual(); 			break;
				case "JSon manual":   pd = new PersonDAO_JSonManual();  break;
				case "YAML":   pd = new PersonDAO_YAMLManual();  		break;
				default:										 		break;
			}
			pp = pd.read();
			fireTableDataChanged();
